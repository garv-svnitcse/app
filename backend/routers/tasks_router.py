...
def _can_touch_task(role, doc, uid):
    ...
    if role == "Employee":
        return (
            doc.get("assignee_id") == uid
            or doc.get("reporter_id") == uid
        )
...
async def update_task(task_id: str, payload: dict, background_tasks: BackgroundTasks, current: UserPublic = Depends(get_current_user)):
    ...
    if current.role == "Intern":
        ...
        } if "status" in payload else {}

        if not payload:
            raise HTTPException(
                403,
                "Interns can only update task status"
            )
...
async def add_comment(
    ...
    if current.role not in ("Founder", "Admin", "Manager"):
        if not (
            doc.get("assignee_id") == current.id
            or doc.get("reporter_id") == current.id
        ):
            raise HTTPException(
                403,
