...
async def _find_user(db, employee_id: str):
    ...
    user = await db.users.find_one(
        {
            "email": {
                "$regex": f"^{re.escape(employee_id_str)}$",
                "$options": "i",
            }
        }
    )
...
async def list_invitations(
    ...
    docs = await db.invitations.find().sort(
        "created_at",
        -1,
    ).to_list(500)
...
async def resend_invitation(
    ...
    # This requires send_invitation_email to exist
    # in your project.
    ...
    return {
        "ok": True,
        "message": (
            f"Invitation email is being resent "
            f"to {inv['email']}"
        ),
    }
...
async def toggle_employee_status(
    ...
    if target.get("role") == "Founder":
        raise HTTPException(
            403,
            "Cannot deactivate the Founder account",
        )
    ...
    if (
        ...
        raise HTTPException(
            403,
            "Only the Founder can deactivate an Admin",
        )
...
async def update_employee(
    ...
    if new_role == "Founder":
        raise HTTPException(
            403,
            "Cannot assign the Founder role",
        )
    ...
    if (
        ...
        raise HTTPException(
            403,
            "Only the Founder can assign the Admin role",
        )
...
async def delete_employee(
    ...
    if target.get("role") == "Founder":
        raise HTTPException(
            403,
            "Cannot remove the Founder account",
        )
    ...
    if (
        ...
        raise HTTPException(
            403,
            "Only the Founder can remove an Admin",
        )
...
async def list_departments(
    ...
    docs = await db.departments.find().sort(
        "name",
        1,
    ).to_list(200)
    ...
    for d in docs:
        ...
        headcount = await db.users.count_documents(
            {
                "department": d["name"]
            }
        )
...
async def add_attendance(
    ...
    today_local = datetime.now().strftime(
        "%Y-%m-%d"
    )
    ...
    if current.role == "Manager":
        ...
        if (
            not tgt
            or tgt.get("department")
            != current.department
        ):
            raise HTTPException(
                403,
                "Managers can only mark attendance "
...
async def create_leave(
    ...
    if current.role == "Manager":
        ...
        if (
            ...
            raise HTTPException(
                403,
                "Managers can only request leave "
                "for their department",
            )
...
async def employees_overview(
    ...
    if current.role in (
        ...
        my_leave = await db.leave_requests.count_documents(
            {
